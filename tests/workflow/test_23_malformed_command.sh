...
if ! command -v jq &>/dev/null || ! command -v node &>/dev/null; then echo "SKIP: test_23 (missing deps)"; exit 0; fi
...
CONTRACT="$EVIDENCE_DIR/test_23_contract.yaml"
cat > "$CONTRACT" << 'EOF'
...
if [ "$EXIT_CODE" -eq 1 ]; then echo "PASS: test_23_malformed_command"; exit 0; else echo "FAIL: test_23_malformed_command — expected exit 1, got $EXIT_CODE"; echo "$output"; exit 1; fi
