...
if ! command -v jq &> /dev/null; then
  echo "SKIP: test_11_unsafe_command — jq not available"
  exit 0
fi
if ! command -v node &> /dev/null; then
  echo "SKIP: test_11_unsafe_command — node not available"
  exit 0
fi
...
CONTRACT="$EVIDENCE_DIR/test_11_contract.yaml"
cat > "$CONTRACT" << 'ENDOFFILE'
CHECKPOINT_ID: CHECKPOINT_3
TITLE: "Test Unsafe Command"
...
if [ "$EXIT_CODE" -eq 0 ]; then
  echo "PASS: test_11_unsafe_command"
  exit 0
else
  echo "FAIL: test_11_unsafe_command — safe command failed with exit $EXIT_CODE"
  exit 1
fi
