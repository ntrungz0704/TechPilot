...
if ! command -v jq &> /dev/null; then
  echo "SKIP: test_10_missing_required_test — jq not available"
  exit 0
fi
if ! command -v node &> /dev/null; then
  echo "SKIP: test_10_missing_required_test — node not available"
  exit 0
fi
...
CONTRACT="$EVIDENCE_DIR/test_10_contract.yaml"
cat > "$CONTRACT" << 'EOF'
CHECKPOINT_ID: CHECKPOINT_3
TITLE: "Test Missing Required Test"
...
ACCEPTANCE_CRITERIA:
  - "Test criterion"
REQUIRED_TESTS:
  - command: "tests/nonexistent_test_script_10.sh"
...
if [ "$EXIT_CODE" -eq 1 ]; then
  echo "PASS: test_10_missing_required_test"
  exit 0
else
  echo "FAIL: test_10_missing_required_test — expected exit 1, got $EXIT_CODE"
  exit 1
fi
