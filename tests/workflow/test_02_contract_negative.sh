...
if ! command -v node &> /dev/null; then
  echo "SKIP: test_02_contract_negative — node not available"
  exit 0
fi
...
CONTRACT="$EVIDENCE_DIR/test_02_contract.yaml"

cat > "$CONTRACT" << 'EOF'
CHECKPOINT_ID: CHECKPOINT_3
TITLE: "Test Contract Missing Evidence"
...
if [ "$EXIT_CODE" -eq 1 ]; then
  echo "PASS: test_02_contract_negative"
  exit 0
else
  echo "FAIL: test_02_contract_negative — expected exit 1, got $EXIT_CODE"
  exit 1
fi
