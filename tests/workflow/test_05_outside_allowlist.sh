...
if ! command -v jq &> /dev/null; then
  echo "SKIP: test_05_outside_allowlist — jq not available"
  exit 0
fi
if ! command -v node &> /dev/null; then
  echo "SKIP: test_05_outside_allowlist — node not available"
  exit 0
fi

if ! git diff --quiet 2>/dev/null || ! git diff --cached --quiet 2>/dev/null; then
  echo "SKIP: test_05_outside_allowlist — working tree has uncommitted changes"
  exit 0
fi
...
CONTRACT="$EVIDENCE_DIR/test_05_contract.yaml"
cat > "$CONTRACT" << 'EOF'
CHECKPOINT_ID: CHECKPOINT_3
TITLE: "Test Outside Allowlist"
...
VIOLATION_FILE="app/test_05_violation.txt"
echo "test violation" > "$VIOLATION_FILE"
git add "$VIOLATION_FILE" > /dev/null 2>&1
git commit -m "test: temporary commit for test_05" > /dev/null 2>&1
...
if [ "$EXIT_CODE" -eq 1 ]; then
  ...
  echo "FAIL: test_05_outside_allowlist — expected exit 1, got $EXIT_CODE"
  exit 1
fi
