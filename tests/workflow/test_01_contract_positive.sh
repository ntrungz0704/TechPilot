...
if ! command -v node &> /dev/null; then
  echo "SKIP: test_01_contract_positive — node not available"
  exit 0
fi
...
CONTRACT="$EVIDENCE_DIR/test_01_contract.yaml"

cat > "$CONTRACT" << 'EOF'
CHECKPOINT_ID: CHECKPOINT_3
TITLE: "Test Contract Positive"
...
if scripts/workflow/validate-contract "$CONTRACT" > /dev/null 2>&1; then
  echo "PASS: test_01_contract_positive"
  rm -f "$CONTRACT"
  exit 0
else
  echo "FAIL: test_01_contract_positive — validate-contract returned non-zero"
  rm -f "$CONTRACT"
  exit 1
fi
