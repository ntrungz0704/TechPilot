...
class SecureCurl
{
    ...
    public static function buildTlsOptions(): array
    {
        ...
        $bundledCert = dirname(__DIR__, 2) . '/config/cacert.pem';
        if (file_exists($bundledCert) && is_file($bundledCert) && is_readable($bundledCert)) {
            $options[CURLOPT_CAINFO] = $bundledCert;
        }
