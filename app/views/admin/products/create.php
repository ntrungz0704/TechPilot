        ...
        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 20px;">
            <div class="form-group">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
                    ...
                    <button type="button" class="btn btn--outline btn--sm" id="btnOpenAiModal" style="border-color: #8B5CF6; color: #7C3AED; background: #F5F3FF; font-weight: 600; padding: 4px 10px; font-size: 12px;" title="Tự động sinh mô tả, thông số kỹ thuật và SEO bằng AI">
    ...
    <div style="background: #FFFFFF; border-radius: 16px; width: 100%; max-width: 720px; max-height: 90vh; overflow-y: auto; box-shadow: 0 25px 50px -12px rgba(0,0,0,0.25); display: flex; flex-direction: column;">
        ...
        <div style="padding: 20px 24px; border-bottom: 1px solid #E2E8F0; display: flex; justify-content: space-between; align-items: center; background: linear-gradient(135deg, #7C3AED, #4F46E5); color: #FFFFFF; border-top-left-radius: 16px; border-top-right-radius: 16px;">
            <div style="display: flex; align-items: center; gap: 12px;">
                ...
                <div>
                    <h3 style="font-size: 17px; font-weight: 700; margin: 0; color: #FFF;">Trợ Lý AI Nhập Sản Phẩm TechPilot</h3>
                    <p style="font-size: 12px; margin: 2px 0 0 0; opacity: 0.9;">Tự động phân tích, tạo mô tả, thông số kỹ thuật & SEO</p>
                </div>
            </div>
        ...
        <div style="padding: 24px; flex: 1;">
            <div style="margin-bottom: 20px;">
                ...
                <div style="display: flex; gap: 10px;">
                    <input type="text" id="aiInputQuery" class="form-control" placeholder="Ví dụ: Laptop Gaming ASUS ROG Strix G16 G614JI" style="flex: 1; padding: 10px 14px; font-size: 14px;">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (openBtn && modal) {
        ...
        applyBtn.addEventListener('click', function() {
            ...
            if (currentAiData.proposed_category) {
                const catSelect = document.getElementById('category_id');
                const targetText = currentAiData.proposed_category.toLowerCase();
                for (let i = 0; i < catSelect.options.length; i++) {
                    if (catSelect.options[i].text.toLowerCase().includes(targetText)) {
            ...
            // Auto select brand if matching
            if (currentAiData.proposed_brand) {
                const brandSelect = document.getElementById('brand_id');
                const targetText = currentAiData.proposed_brand.toLowerCase();
                for (let i = 0; i < brandSelect.options.length; i++) {
                    if (brandSelect.options[i].text.toLowerCase().includes(targetText)) {
