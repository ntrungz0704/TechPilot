<div class="card">
    ...
    <form method="get" action="<?= url('admin/users') ?>" style="margin-bottom: 25px; display: flex; gap: 15px; flex-wrap: wrap;">
        ...
        <select name="role_id" class="form-control" style="max-width: 180px;">
            <option value="">Tất cả chức vụ</option>
            <option value="1" <?= $roleId === '1' ? 'selected' : '' ?>>Quản trị viên (Admin)</option>
            <option value="2" <?= $roleId === '2' ? 'selected' : '' ?>>Khách hàng (Customer)</option>
        </select>
                ...
                <?php if (!empty($users)): ?>
                    <?php foreach ($users as $u): ?>
                        <tr>
                            ...
                            <td>
                                <span class="badge" style="background-color: <?= $u['role_id'] == 1 ? '#FEE2E2; color: #991B1B;' : '#E0F2FE; color: #0369A1;' ?>">
                            ...
                            <td style="text-align: center; display: flex; gap: 8px; justify-content: center; align-items: center; height: 50px;">
                                ...
                                <form method="post" action="<?= url('admin/users/change_role/' . $u['id']) ?>" style="margin: 0; display: flex; gap: 4px; align-items: center;">
                                    <?= csrf_field() ?>
                                    <select name="role_id" style="padding: 6px; border: 1px solid var(--border); border-radius: 4px; font-size: 12px; font-weight: 600;">
                                        <option value="2" <?= $u['role_id'] == 2 ? 'selected' : '' ?>>Customer</option>
