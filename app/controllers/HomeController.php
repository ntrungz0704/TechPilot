...
class HomeController extends Controller
{
    ...
    public function index(): void
    {
        ...
        $data = [
            ...
            'newProducts'            => $productModel->getNewProducts(12),
            'promoProducts'          => $productModel->getPromoProducts(12),

            // Các danh mục sản phẩm lớn ở trang chủ
            'laptopGaming'           => $productModel->search('', 'laptop-gaming', 12) ?: [],
            'laptopVanPhong'         => $productModel->search('', 'laptop-van-phong', 12) ?: [],
            'pcBuildSan'             => $productModel->getByCategorySlug('pc-build-san', 12),
            'pcLinhKien'             => $productModel->getByCategorySlug('pc-linh-kien', 12),
            'gamingGear'             => $productModel->getByCategorySlug('gaming-gear', 12),
            'monHinh'                => $productModel->getByCategorySlug('man-hinh', 12),
            'apple'                  => $productModel->getByCategorySlug('may-tinh-bo', 12),

            // Dữ liệu cho Best Seller Tabs
            'bestSellersLaptop'      => $productModel->getBestSellersByTab('laptop', 12),
            'bestSellersGaming'      => $productModel->getBestSellersByTab('gaming', 12),
            'bestSellersComponents'  => $productModel->getBestSellersByTab('components', 12),
            'bestSellersMonitor'     => $productModel->getBestSellersByTab('monitor', 12),
            'bestSellersAccessories'  => $productModel->getBestSellersByTab('accessories', 12),
